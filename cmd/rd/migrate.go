...
var migrateCmd = &cobra.Command{
	Use:   "migrate",
	Short: "Re-emit the campfire item set as nostr events; --parity verifies the migration",
	Long: `Migrate this project's campfire/JSONL work items to the nostr-native backend.
...
preserved. Migration is NON-DESTRUCTIVE: the campfire source is left intact and
remains the default backend. Idempotent by event id (safe to re-run).

--parity: do NOT migrate. Instead assert item-for-item parity between the campfire
...
func init() {
	...
	migrateCmd.Flags().Bool("parity", false, "assert item-for-item parity (campfire source == nostr projection) and exit non-zero on any mismatch, instead of migrating")
