...
info "STEP 1: create + claim + done an item through RD_NOSTR=1 (publishes to $RELAY_URL)"
...
ID="$(cd "$PROJ" && CF_HOME="$CFHOME" RD_NOSTR=1 RD_NOSTR_RELAY_URL="$RELAY_URL" "$RD" create "f92 causal-ordering proof" --type task --priority p1 --context "ready-f92" 2>>"$WORK/a.err" | tail -1)"
[ -n "$ID" ] || { cat "$WORK/a.err" >&2; fail "rd create produced no item id"; }
info "item id: $ID"
sleep 1
( cd "$PROJ" && CF_HOME="$CFHOME" RD_NOSTR=1 RD_NOSTR_RELAY_URL="$RELAY_URL" "$RD" claim "$ID" >/dev/null 2>>"$WORK/a.err" ) || true
sleep 1
( cd "$PROJ" && CF_HOME="$CFHOME" RD_NOSTR=1 RD_NOSTR_RELAY_URL="$RELAY_URL" "$RD" done "$ID" --reason "shipped: f92 proof" >/dev/null 2>>"$WORK/a.err" ) || true
...
mkdir -p "$P1" "$P2"
( cd "$P1" && CF_HOME="$CFHOME" "$RD" init --offline >/dev/null )
( cd "$P2" && CF_HOME="$CFHOME" "$RD" init --offline >/dev/null )
V1="$(cd "$P1" && CF_HOME="$CFHOME" RD_NOSTR=1 RD_NOSTR_RELAY_URL="$RELAY_URL" "$RD" nostr show "$ID" --reconcile 2>>"$WORK/a.err")"
V2="$(cd "$P2" && CF_HOME="$CFHOME" RD_NOSTR=1 RD_NOSTR_RELAY_URL="$RELAY_URL" "$RD" nostr show "$ID" --reconcile 2>>"$WORK/a.err")"
printf '%s\n' "$V1" | sed 's/^/    [log1] /'
if [ "$V1" != "$V2" ]; then
  printf '%s\n' "$V2" | sed 's/^/    [log2] /'
...
( cd "$P1" && CF_HOME="$CFHOME" RD_NOSTR=1 RD_NOSTR_RELAY_URL="$RELAY_URL" "$RD" nostr show "$ID" --reconcile >/dev/null 2>>"$WORK/a.err" ) || true
