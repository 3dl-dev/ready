...
#      Go signer AND with nak; assert BOTH id and sig match byte-for-byte.
#   2. Live loop: with a FRESH generated key, build+sign an event in Go, publish
#      it to a live relay via the Go publisher (relay answers OK,true), read it
#      back, independently Verify (ACCEPT), then tamper a byte and Verify
#      (REJECT).
#
...
# key used only to make the cross-check reproducible.
#
...
PROVE_OUT="$("${DEMO[@]}" prove --relay "$RELAY")"
printf '%s\n' "$PROVE_OUT"
