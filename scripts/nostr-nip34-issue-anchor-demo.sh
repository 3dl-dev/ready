...
cd "$PROJ"
info "rd init --offline (JSONL-only project; nostr NOT enabled for create/close)"
"$RD" init --offline >/dev/null

echo
info "STEP 1: rd create (no RD_NOSTR -- item lives ONLY in campfire/JSONL so far)"
...
[ ! -f "$LOG" ] || fail "nostr log should not exist yet (nostr was never enabled)"

echo
info "STEP 2: rd done --reason (still no nostr -- reason lives only in local history)"
...
echo
info "STEP 3: rd nostr publish $ID -- FIRST-EVER nostr publish, via the MANUAL path"
export RD_NOSTR_RELAY_URL="$RELAY"
PUBLISH_OUT="$("$RD" nostr publish "$ID" 2>&1)" || fail "rd nostr publish failed: $PUBLISH_OUT"
echo "$PUBLISH_OUT" | sed 's/^/     /'
echo "$PUBLISH_OUT" | grep -q "^published kind 1621 " || fail "no kind:1621 issue-root event published"
echo "$PUBLISH_OUT" | grep -q "^published kind 1631 " || fail "no kind:1631 (resolved) status event published"
[ -f "$LOG" ] || fail "local nostr log not created by the manual publish"
...
pass "close-with-reason survived the manual \`rd nostr publish\` path (ready-da7 fix #2)"
...
SUMMARY
  ...
  proof:   (1) status events anchor to BOTH the 30302 card (rd's own read, unchanged)
           ...
           (2) \`rd nostr publish\` carries the item's recorded close reason through
               on a first-ever/manual republish (previously hand-carried "")
EOF
