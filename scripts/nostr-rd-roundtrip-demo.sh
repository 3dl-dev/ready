...
#
# Steps:
#   1. `rd create` an item with RD_NOSTR=1 -> publishes a 30302 card + 1630 status
...
echo
info "STEP 1: rd create with RD_NOSTR=1 -> publish 30302 card + 1630 status to the LIVE relay + local log"
ID="$(RD_NOSTR=1 "$RD" create "keystone round-trip" --type task --priority p1 --context "ready-a13 live proof" 2>"$WORK/create.err" | tail -1)"
...
CLEAN_OUT="$(RD_NOSTR=1 "$RD" nostr show "$ID" --reconcile)"
printf '%s\n' "$CLEAN_OUT" | sed 's/^/    /'
