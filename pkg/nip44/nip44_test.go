...
// VENDORED from dontguess pkg/nip44/nip44_test.go. The only change is the
// key port: identity.FromPrivHex/identity.Generate → nostr.KeyFromHex/
// nostr.GenerateKey, since *nostr.Key is rd's ecdher. No KAT case is weakened,
// deleted, or skipped — a failing case means the ECDH/nip44 port is wrong.
//
// The conversation-key KATs derive every key THROUGH *nostr.Key.ECDH, so a
// green suite is external-ground-truth proof rd's ECDH accessor returns the
// correct raw shared-X (a symmetry test alone cannot catch a wrong KDF or a
// sha256(X) mistake).
...
// vectors mirrors the official paulmillr/nip44 nip44.vectors.json layout. Only
// the v2 classes this package owns are decoded.
type vectors struct {
	V2 struct {
		Valid struct {
...
// ecdher port: KeyFromHex(sec1).ECDH(pub2) → HKDF-Extract must equal the
...
func TestKAT_ValidGetConversationKey(t *testing.T) {
	...
	for i, tc := range v.V2.Valid.GetConversationKey {
		id, err := nostr.KeyFromHex(tc.Sec1)
		if err != nil {
			t.Fatalf("case %d: KeyFromHex(%s): %v", i, tc.Sec1, err)
		}
...
// vector it: (1) re-derives conversation_key via the ecdher port from sec1/sec2
...
func TestKAT_ValidEncryptDecrypt(t *testing.T) {
	...
	for i, tc := range v.V2.Valid.EncryptDecrypt {
		...
		// (1) conversation key through the ecdher port (sec1 is us, sec2's pubkey
		// is the counterparty x-only key).
		id1, err := nostr.KeyFromHex(tc.Sec1)
		if err != nil {
			t.Fatalf("case %d: KeyFromHex(sec1): %v", i, err)
		}
		id2, err := nostr.KeyFromHex(tc.Sec2)
		if err != nil {
			t.Fatalf("case %d: KeyFromHex(sec2): %v", i, err)
		}
...
// TestSealOpenRoundTrip exercises the full public API through the ecdher port
...
func TestSealOpenRoundTrip(t *testing.T) {
	alice, err := nostr.GenerateKey()
	if err != nil {
		t.Fatalf("generate alice: %v", err)
	}
	bob, err := nostr.GenerateKey()
	if err != nil {
...
func TestSealNonceIsRandom(t *testing.T) {
	alice, _ := nostr.GenerateKey()
	bob, _ := nostr.GenerateKey()
