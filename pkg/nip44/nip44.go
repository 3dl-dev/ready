...
//
// VENDORED from dontguess pkg/nip44 (github.com/3dl-dev/dontguess) for the
// confidential-boards epic (ready-216). It is a COPY, not a dependency: import
// paths are rewritten to rd's module and the identity.Signer port is replaced by
// the minimal local `ecdher` interface (below), which *nostr.Key satisfies.
//
// It is the wrap primitive for confidential rd boards: the 32-byte per-board
// content-encryption key (CEK) is NIP-44-wrapped to each granted member inside
// the owner-signed role grant (see the keydist item). This package is
// deliberately a thin, testable surface — it does NOT touch rd event/sync/board
// logic or the CEK/AEAD bulk-content path (those are the write/read items). It
// exposes exactly two public operations, Seal and Open, both of which derive the
// NIP-44 conversation key via ecdher.ECDH (the raw shared X coordinate; see
// pkg/nostr/ecdh.go).
//
// CRITICAL: this is security-critical crypto validated byte-for-byte against the
// official NIP-44 v2 known-answer vectors (paulmillr/nip44, testdata/
// nip44.vectors.json) in nip44_test.go. Do not "simplify" the padding, KDF, or
// MAC construction without re-running the full KAT — the vectors exist to catch
// exactly those silent mistakes.
package nip44
...
// ecdher is the minimal key-agreement port this package needs. *nostr.Key
// satisfies it (see pkg/nostr/ecdh.go). ECDH returns the RAW 32-byte big-endian
// shared X coordinate (NOT sha256(X)); a wrong return breaks decryption
// silently, which is why the conversation-key KATs derive through this port.
...
func Seal(signer ecdher, counterpartyXOnlyHex string, plaintext []byte) (string, error) {
...
func Open(signer ecdher, counterpartyXOnlyHex string, payload string) ([]byte, error) {
	convKey, err := conversationKey(signer, counterpartyXOnlyHex)
	if err != nil {
		return nil, err
	}
...
// the ecdher port. signer.ECDH returns the raw 32-byte shared X coordinate;
// conversationKeyFromShared runs HKDF-Extract over it.
func conversationKey(signer ecdher, counterpartyXOnlyHex string) ([32]byte, error) {
