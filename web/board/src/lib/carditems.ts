...
//
// ⚠ NOT ON THE RENDER PATH AS OF ready-56b. When c4b was merged, ready-35b's
// CONFORMANT fold (lib/fold.ts — 36 Go-generated vectors, byte-for-byte) had
// landed and already accepted a decryptor, so main.ts wires the keyring into
// THAT and this module projects nothing the page shows. It is retained because
// its 18 tests are a genuine second reading of the gate ORDER against the same
// fixtures, which is a real cross-check on fold.ts. It must not be re-adopted
// as a render path: ../board/types.ts's header states the rule this file is
// the exception to — a second event->Item fold duplicates the conformance
// surface and can silently diverge from it. If it ever costs more than it
// proves, delete it and its suite together; do not quietly wire it back in.
//
