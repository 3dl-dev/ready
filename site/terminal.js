...
(function() {
  ...
  var DEMOS = {
    ...
    isolation: {
      title: 'Multi-actor attribution',
      subtitle: '$RD_HOME / $RD_ACTOR',
      source: 'https://github.com/3dl-dev/ready/blob/main/scripts/nostr-sign-demo.sh',
      lines: [
        { type: 'comment', text: '# Identity is a secp256k1 key in $RD_HOME (default ~/.config/rd).' },
        { type: 'comment', text: '# No shared server, no config daemon \u2014 the signed log IS the board.' },
        { type: 'output', text: '~/.config/rd/' },
        { type: 'output', text: '  nostr-identity.json          \u2190 owner key' },
        { type: 'output', text: '  keys/agent-pm.json           \u2190 agent:pm key ($RD_ACTOR)' },
        { type: 'output', text: 'myproject/' },
        { type: 'output', text: '  .ready/nostr-log.jsonl       \u2190 signed events (the board)' },
        { type: 'blank' },
        { type: 'comment', text: '# Walk up from anywhere in the project. Owner claims one item.' },
        { type: 'cmd', text: 'cd myproject' },
        { type: 'cmd', text: 'rd claim myproject-2ed' },
        { type: 'output', text: 'claimed myproject-2ed' },
        { type: 'blank' },
        { type: 'comment', text: '# $RD_ACTOR selects a DISTINCT key \u2014 signs as agent:pm' },
        { type: 'cmd', text: 'RD_ACTOR=agent:pm rd claim myproject-131' },
        { type: 'output', text: 'claimed myproject-131' },
        { type: 'blank' },
        { type: 'comment', text: '# Every transition is attributed to the signing key' },
        { type: 'cmd', text: 'rd show myproject-2ed | grep By:' },
        { type: 'output', text: 'By:       a9f766ae56bbf466d2d361e5b1788b7cd689fd8e3b418e35b002b313f478db25' },
        { type: 'cmd', text: 'rd show myproject-131 | grep By:' },
        { type: 'output', text: 'By:       24a6e814e437b4aae282a13f8fc0625a0dfb6dcb800dbe408ca47fa4255632d2' },
      ]
    }
  };
