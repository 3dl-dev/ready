...
(function() {
  ...
  var DEMOS = {
    solo: {
      title: 'The core loop',
      subtitle: 'create · claim · close',
      source: 'https://github.com/3dl-dev/ready/blob/main/scripts/nostr-rd-roundtrip-demo.sh',
      lines: [
        { type: 'comment', text: '# Start a project. No server, no signup. Confidential by default.' },
        { type: 'cmd', text: 'rd init --name myproject' },
        { type: 'output', text: 'initialized myproject (confidential)' },
        { type: 'output', text: '  board: 30301:a9f766ae56bb...:myproject' },
        { type: 'output', text: '  log:   .ready/nostr-log.jsonl' },
        { type: 'blank' },
        { type: 'comment', text: '# Create an item. Signed event, appended to the log.' },
        { type: 'cmd', text: 'rd create "Ship login page" --priority p1 --type task' },
        { type: 'output', text: 'myproject-6b7' },
        { type: 'blank' },
        { type: 'comment', text: '# What needs me right now?' },
        { type: 'cmd', text: 'rd ready' },
        { type: 'output', text: '  myproject-6b7     p1        inbox                   Ship login page' },
        { type: 'blank' },
        { type: 'comment', text: '# Grab it. You are the performer now.' },
        { type: 'cmd', text: 'rd claim myproject-6b7' },
        { type: 'output', text: 'claimed myproject-6b7' },
        { type: 'blank' },
        { type: 'comment', text: '# Close it with a reason. The reason is the point.' },
        { type: 'cmd', text: 'rd done myproject-6b7 --reason "Login page ships with JWT auth"' },
        { type: 'output', text: 'closed myproject-6b7 (done)' },
      ]
    },
    confidential: {
      title: 'Confidential boards',
      subtitle: 'encrypted · member-only',
      source: 'https://github.com/3dl-dev/ready/blob/main/docs/design/confidential-boards-envelope.md',
      lines: [
        { type: 'comment', text: '# New boards seal their free text by default.' },
        { type: 'cmd', text: 'rd init --name vault' },
        { type: 'output', text: 'initialized vault (confidential)' },
        { type: 'output', text: '  free text is ENCRYPTED — only granted members can read it.' },
        { type: 'output', text: '  relays cache ciphertext, never plaintext.' },
        { type: 'blank' },
        { type: 'comment', text: '# Create something you would not want on a shared relay.' },
        { type: 'cmd', text: 'rd create "Rotate the leaked Stripe key" --priority p0 --type task' },
        { type: 'output', text: 'vault-2ef' },
        { type: 'blank' },
        { type: 'comment', text: '# You wrote it, so you read it — plaintext, zero key handling.' },
        { type: 'cmd', text: 'rd show vault-2ef' },
        { type: 'output', text: 'Title:    Rotate the leaked Stripe key' },
        { type: 'output', text: 'Status:   inbox' },
        { type: 'output', text: 'Priority: p0' },
        { type: 'blank' },
        { type: 'comment', text: '# Someone NOT on the board pulls the same events. They get nothing:' },
        { type: 'cmd', text: 'rd show vault-2ef' },
        { type: 'output', text: 'Title:    [encrypted]' },
        { type: 'blank' },
        { type: 'comment', text: '# Add a teammate. The board key rides inside the signed grant.' },
        { type: 'cmd', text: 'rd grant e9f1a5b3a57b... contributor' },
        { type: 'output', text: 'published role-grant: grantee=e9f1a5b3a57b... role=contributor' },
        { type: 'blank' },
        { type: 'comment', text: '# Now they can read it too. Membership IS the read key.' },
        { type: 'cmd', text: 'rd show vault-2ef' },
        { type: 'output', text: 'Title:    Rotate the leaked Stripe key' },
        { type: 'blank' },
        { type: 'comment', text: '# Revoke and the key rotates. They keep the past, lose the future.' },
        { type: 'cmd', text: 'rd revoke e9f1a5b3a57b...' },
        { type: 'output', text: 'published role-grant: grantee=e9f1a5b3a57b... role=revoked' },
        { type: 'comment', text: '# Every item written after this is sealed under a fresh key.' },
      ]
    },
    team: {
      title: 'Team: follow + grant',
      subtitle: 'one command, keeps your key',
      source: 'https://github.com/3dl-dev/ready/blob/main/scripts/nostr-grant-revoke-demo.sh',
      lines: [
        { type: 'comment', text: '# Teammate follows you from their OWN machine — keeps their key,' },
        { type: 'comment', text: '# no invite token to copy, no coordinate to retype.' },
        { type: 'cmd', text: 'rd follow baron@3dl.dev' },
        { type: 'output', text: 'found 2 boards owned by baron@3dl.dev: backend, frontend' },
        { type: 'output', text: 'bound board 30301:a9f766ae56bb...:backend -> .ready/board.json' },
        { type: 'output', text: 'your pubkey: 73fd6d99fa59...' },
        { type: 'output', text: 'send the owner: rd grant --all-boards 73fd6d99fa59...' },
        { type: 'blank' },
        { type: 'comment', text: '# Owner runs ONE command — role is optional, defaults to contributor' },
        { type: 'cmd', text: 'rd grant --all-boards 73fd6d99fa59...' },
        { type: 'output', text: 'published role-grant: grantee=73fd6d99fa59... role=contributor (2 boards)' },
        { type: 'blank' },
        ...
        { type: 'blank' },
        { type: 'comment', text: '# Prefer a one-use token instead of sharing an email/npub? Still works:' },
        { type: 'cmd', text: 'rd invite' },
        { type: 'output', text: 'rd1_eyJ2IjozLCJib2FyZCI6IjMwMzAxOmE5Zjc2NmFlNTZiYmY0Ni...  (one-use, 2h)' },
    ...
    gate: {
      ...
      source: 'https://github.com/3dl-dev/ready/blob/main/scripts/nostr-write-path-demo.sh',
      lines: [
    ...
    isolation: {
      ...
      lines: [
        ...
        { type: 'output', text: 'published role-grant: grantee=3933c96d7fe0... role=contributor board=30301:a9f766ae56bb...:myproject' },
        { type: 'output', text: '  event id=e4c07e2326a9... relay-accepted=true' },
        { type: 'blank' },
        { type: 'comment', text: '# Self-hosted / locked relays: regenerate + push the write-allowlist' },
        { type: 'cmd', text: 'rd relay sync-allowlist --apply' },
        { type: 'output', text: 'relay write-allowlist regeneration (board author a9f766ae56bb...)' },
        { type: 'output', text: '  baseline: 1 key(s)  ->  final: 2 key(s)' },
        { type: 'blank' },
